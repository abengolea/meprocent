...
import { Intervencion, Equipo, User, Alarma } from "@/lib/types";
...
export default function DashboardPage() {
    ...
    const alarmasQuery = useMemo(() => {
        if (!db || !profile) return null;
        if (profile.role === 'super_admin') {
            return query(collection(db, "alarmas"));
        }
    ...
    const { data: usuarios } = useCollection<User>(usuariosQuery);
